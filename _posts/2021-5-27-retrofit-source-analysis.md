...
下图中，黑线部分是创建`ServiceMethod`的过程，红线部分是`invoke()`的调用过程。

![Retrofit流程](https://malinkang-1253444926.cos.ap-beijing.myqcloud.com/blog/images/Retrofit流程.png)

`ServiceMethod`创建过程：

1. 通过动态代理，获取到我们定义的接口里的所有`Method`，调用`ServiceMethod`的静态方法`parseAnnotations`创建`ServiceMethod`。
2. `ServiceMethod`调用`RequestFactory`的静态方法`parseAnnotations`创建`RequestFactory`。
3. `RequestFactory`解析每个方法上参数注解信息并封装到`ParameterHandler`中。
4. `ServiceMethod`调用`HttpServiceMethod`的静态方法`parseAnnotations`。
5. `parseAnnotations`从`Retrofit`中获取`CallAdapter`，并创建`CallAdapted`。

`ServiceMethod`的`invoke()`执行流程：

1. 调用`ServiceMethod`的`invoke()`。`invoke()`会调用`CallAdapted`的`invoke()`。
2. `CallAdapted`的`invoke()`创建`OkHttpCall`，并调用`CallAdapter`的`adapt()`。
3. `CallAdapter`调用`OkHttpCall`的`enqueue()`。
4. `OKHttpCall`调用`RequestFactory`的`create()`方法构建`Request`。
5. `OkHtppCall`执行网络请求。
...
![ServiceMethod类的继承关系](https://malinkang-1253444926.cos.ap-beijing.myqcloud.com/blog/images/ServiceMethod.png)
...
`RequestFactory`负责获取`Method`上的注解信息，并装成成`Request`。
...
`Builder`类是`RequestFactory`的内部类，负责构建`RequestFactory`。
...
`parseParameterAnnotation()`比较长，总体的解析操作都大同小异，我这里只解析`@Query`。
...
`RequestFactory`的`create()`方法会遍历`ParameterHandler`数组，构建`Request`。该方法会被`OKHttpCall`调用。
...
`OkHttpCall`继承自`Call`。`Call`是一个接口，提供了如下方法：

| 方法名                                | 方法说明     |
| ------------------------------------- | ------------ |
| `Response<T> execute();`              | 执行异步请求 |
| `void enqueue(Callback<T> callback);` | 执行同步请求 |
| `void cancel();`                      | 取消请求     |
